  ...
  if (!quiet) {
    try {
      const reply = await newThread.postToUser(`Welcome to the /r/Overwatch CoachMail bot. This bot is intended for use by players who ` +
        ...
        `videos or replays. If you simply have questions about heroes, maps, or any other aspect of the game, check out ` +
        `<#${config.adviceChannel}>, where coaches and community members can help you out in a more casual format. If this applies to you, ` +
        `choose ❌ below to cancel the session. If you would like to continue, answer the following questions by reacting to emoji underneath.
